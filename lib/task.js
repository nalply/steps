...
function createTask(request, response, config, steps) {
  return new Task(request, response, config, steps);
}

function Task(request, response, config, steps) {
  this.request = Object.freeze(request);
  this.response = responseShim(response);  
  ...
  var store = {};
  this.getStore = function() { return store } // todo make read only
  this.getRunStore = function() { return store } // will be deleted in start()
}
...
Task.prototype.store = _store; // maybe getStore() and also getConfig()
                               // rename getStore above to getAllStore
function responseShim(response) {
  var shim = {};
...
// Todo replace utils.fatal() by something which sends 500 (needs response)
function _start() {
  var start = this.allSteps[this.config.server.start];
  if (!start) utils.fatal("Step " + this.config.server.start + " unavailable");
  
  var store = this.getRunStore();
  delete this.getRunStore;
  Object.freeze(this);

  this.run([start], store, function(err) {
...
function _run(steps, store, cb) {
  ...
  steps[0](that, function(err, param) {
    ...
    if (param === STORE) {
      if (!store[steps[0].stepId]) store[steps[0].stepId] = {};
      return store[steps[0].stepId];
    }
    ...
    if (param === NEXT) {
      ...
      return that.run(steps, store, cb);
    }
