...
function loadSteps(config, dirs) {
  var steps = {};
  var dirname = path.resolve(path.dirname(config.config_dirname));
  
  for (var d = 0; d < dirs.length; d++) {
    var files = fs.readdirSync(dirname + '/' + dirs[d]);
    
    if (DBG) console.log('Loading steps from', dirs[d]);
    if (DBG) console.log('  files:', files.join(" "));
    
    for (var f = 0; f < files.length; f++) doStep(dirs[d], files[f]);
  }
  return steps;
  
  // TODO don't allow overwrites
  // TODO replace / by . for subdirectories
  function doStep(dir, file) {
    var base = path.basename(file, '.js');
    var preId = dir + (base == 'index' ? '' : '.' + base);
    var fullPath = dirname + '/' + dir + '/' + file;
